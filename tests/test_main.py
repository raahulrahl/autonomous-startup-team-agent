...
async def test_handler_returns_response():
    ...
    with (
        patch("autonomous_startup_team_agent.main._initialized", True),
...
async def test_handler_with_multiple_messages():
    ...
    with (
        patch("autonomous_startup_team_agent.main._initialized", True),
        patch(
            "autonomous_startup_team_agent.main.run_agent", new_callable=AsyncMock, return_value=mock_response
        ) as mock_run,
    ):
        result = await handler(messages)
...
async def test_handler_initialization():
    ...
    with (
        patch("autonomous_startup_team_agent.main._initialized", False),
